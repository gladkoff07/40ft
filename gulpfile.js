...
const config = {
  ...
  download: {
    src: `${coreDir.src}/download/*`,
    dist: `${coreDir.dist}/download`,
    watch: `${coreDir.src}/download/**/*`,
  },
  video: {
    src: `${coreDir.src}/video/*`,
    dist: `${coreDir.dist}/video`,
    watch: `${coreDir.src}/video/**/*`,
  },
  scripts: {
...
/* Download */
gulp.task("download", () => {
  return gulp.src(config.download.src).pipe(gulp.dest(config.download.dist));
});

/* Video */
gulp.task("video", () => {
  return gulp.src(config.video.src).pipe(gulp.dest(config.video.dist));
});
...
gulp.task("deploy", function () {
  ...
  return gulp
    ...
    .pipe(conn.newer("www/html/team-orange.ru/sites/40ft/")) // only upload newer files
    .pipe(conn.dest("www/html/team-orange.ru/sites/40ft/"));
});
